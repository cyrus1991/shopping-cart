class shoppingCart {
  ...
  init(){
    ...
    for (var i in database ) {
     ...
     let currencySymbul = "$";
     var newShipping = database[i].shipping * 1.23;

      if (window.location.search.substring(1)=== "lang=de"){
        currencySymbul = "€";
        s.innerHTML = `shipping: ${database[i].shipping + currencySymbul} <br> delivery: ${database[i].delivery}d`
      } else{
        s.innerHTML = `shipping: ${(database[i].shipping*1.23).toFixed(2) + currencySymbul} <br> delivery: ${database[i].delivery}d`
      } 
  ...
  render(){
    ...
    this.db.items.forEach( item => {
      ...
      if (window.location.search.substring(1)=== "lang=de"){
        ...
        currencySymbul = "€";
        listItem.innerHTML = `<span class="badge badge-info badge-pill mr-2">${item.count} </span>  ${item.name} - ${item.price + currencySymbul} <span class="ml-auto mr-3 font-weight-bold">${(item.price * item.count).toFixed(2) + currencySymbul}</span>`;
      }  else{

                listItem.innerHTML = `<span class="badge badge-info badge-pill mr-2">${item.count} </span>  ${item.name} - ${(item.price * 1.23).toFixed(2)+ currencySymbul} <span class="ml-auto mr-3 font-weight-bold">${((item.price * 1.23)  * item.count).toFixed(2) + currencySymbul}</span>`;
